...
\paragraph{Minimizers of errors.}
...
$L^2(\rho)$. However, the matrix on the left is not samplable. We need
double sampling for that.

Combined with all this, is the problem that a Bellman solution on the
...
problem (we can sample $\transp{(\rho\Delta)}\rho\Delta$, but in practice the sample will always be $0$
unless we are lucky and select two transitions $s\to s'$ that have the
same $s$). Even the right-hand-side involves double sampling.
...
in the linear case. The features should not be orthogonal. Should use a
...
$\eps$.
\item For $\eps$ small enough, iterating $\theta\gets (\Id-\eps A)\theta$
converges to $0$.
...
In the reversible case, $\mu\Delta$ is symmetric. We can use the
...
In the non-reversible case, let $\mu\Delta=S+A$ with $S$ symmetric and
$A$ antisymmetric. Let $L\deq \mu S^{-1} \mu$. Then we have to prove that
$\mu S^{-1} \mu \Delta \Phi B\transp{\Phi}\rho$ is positive. For any
function $f$,
\begin{align}
\transp{f} \mu S^{-1} \mu \Delta \Phi B\transp{\Phi}\rho f
&=\transp{f} \mu S^{-1} (S+A) \Phi B\transp{\Phi}\rho f
\\&=\transp{f} \mu \Phi B\transp{\Phi}\rho f + \transp{f} \mu S^{-1}
A \Phi B\transp{\Phi}\rho f
\end{align}

Let us consider the on-policy case. Then the first term is $\transp{f}
\mu \Phi B\transp{\Phi}\rho f$ and is always positive. For the second
term, since $A$ is antisymmetric, we have $\transp{x}Ax=0$ for all $x$.
Thus, we can add any multiple of $\transp{x}Ax=0$; let us take
$x=S^{-1}\mu f$:
\begin{equation}
\transp{f} \mu S^{-1}
A \Phi B\transp{\Phi}\mu f=
\transp{f} \mu S^{-1}A (S^{-1}-\Phi B\transp{\Phi})\mu f
\end{equation}
...
Thus we have a two-timescale algorithm for non-linear TD in the on-policy
