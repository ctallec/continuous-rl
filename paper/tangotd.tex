...
apply the inverse Dirichlet norm to $((\Id-P)V-R)$. When When $P$ is
reversible and $\rho=\mu$ this
is the same. One tries to minimize
$\transp{((\Id-P)V-R)}\rho\mathcal{D}^{-1}\rho((\Id-P)V-R)$ where
$\mathcal{D}\deq \rho+\rho'-\rho P-\transp{(\rho P)}$ as above, and use
TANGO to invert $\mathcal{D}$. However this suffers from a double
sampling problem.
