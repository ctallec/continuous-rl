...
featurize/dualize
by asking that this is satisfied for any functions on the left and right.
}
...
contains all the necessary to do policy gradient or actor-critic with
arbitrary reward functions, as seen above. It is feasible to store a
...
\end{equation}
which represent that, when observing a transition $s\to s'$, we update
both the ways to arrive at $s'$ and the ways to start at $s$.

``Universal value function approximators'' (Schaul--Horgan--Gregor--Silver ICML 2015) do a bit of that, but only
with the left (ordinary) Bellman equation. It amounts to solving $\#S$
independent Bellman equations, and relying on the generalization
...
\begin{equation}
\label{eq:greenrep}
\E_{s_2\sim \rho}\,\green(s_1)\cdot
f(s_2)\psi(s_2)\approx \transp{\1_{s_1}}\,\Delta^{-1} \psi
\end{equation}

If the state space is finite and if we use a one-hot representation, a
solution is indeed $E=\R^{\#S}$ with
\begin{equation}
f(s_2)=\1_{s_2},\qquad
(\green(s_1))_{s_2}=(\Delta^{-1})_{s_1s_2}/\rho(s_2)
\end{equation}

The reason for this form is the following: $E$ is supposed to represent
distributions/functions over states. $f(s_2)$ is supposed to be
mapped to $\1_{s_2}$. $\green(s_1)$ is supposed to be mapped to the
density wrt $\rho$ of the fundamental matrix at $s_1$ (the occupation
distribution from $s_1$: $\transp{\1_{s_1}}\sum_t P^t$, where as usual I
omit constants\footnote{To deal with constants, we probably have to be
able to predict \emph{differences} $\psi(s_2)-\psi(s_2')$.}). In
particular, $\E_{s_2\sim \rho}f(s_2)\psi(s_2)$ is supposed to represent
the vector with components $\rho \psi(s_2)$. Then the scalar product in
$E$ represents integration/summation with respect to $\rho$.
...
learn. But we can use the Bellman equations \eqref{eq:greenbell}; after
...
\E_{s_2\sim \rho}\,\green(s_1)\cdot
f(s_2)\psi(s_2)
\approx \psi(s_1)+ \E_{s'\sim P(s'|s_1)}\E_{s_2\sim
\rho}\,\green(s')\cdot
f(s_2)\psi(s_2)
\end{gather}
and the right Bellman equation becomes
\begin{gather}
...
$(P\psi)(s_2)=\E_{s'_2\sim P(s'_2|s_2)}\psi(s_2)$).
