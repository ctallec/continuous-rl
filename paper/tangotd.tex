...
$\rho\Delta$, namely, $\rho\Delta=\E_{s,s'} 1_s\transp{(\1_s-\gamma
\1_{s'})}$. This corresponds to a stochastic version of the Newton
method, with learning rate $\eta\ll 1$,
\begin{equation}
A\gets A+\eta A-\eta A 1_s\transp{(\1_s-\gamma \1_{s'})} A
\end{equation}
Indeed, in expectation over $s\to s'$ this corresponds to $A\gets A+\eta
A-\eta A \rho\Delta A$, which converges to $A=\Delta^{-1}\rho^{-1}$. This
...
if we update our estimate of $\rho\Delta$ by
$\rho\Delta\gets (1-\eta)\rho\Delta+\eta 1_s\transp{(\1_s-\gamma
\1_{s'})}$ when a transition $s\to s'$ is observed, then
$(\rho\Delta)^{-1}$ gets updated as above.
