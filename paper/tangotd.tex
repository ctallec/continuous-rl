...
Bellman solution.  This is an absolutely major problem off-policy.
...
in $V(s+1)$ for $s\geq 1$ and in $V(s-1)$ for $s\leq -1$.
...
solution, for any linear function $f$.

What happens with linear functions is that the system ``believes''
...
For instance, if the reward is $0$ at $0$ and $-1$ everywhere else, the
...
and the value function is $V(s)=\abs{s}-n$, shaped like a ``V''.
...
solution everywhere except at $s=0$.
...
exactly everywhere except at $s_0$. Off-policy these violations would not be
...
discarded the possibility to stay in place). [One gets a constraint that
rewards are negative because in such a situation, the optimal policy
...
reward so that the maximum reward becomes $0$.]
...
early TD-like optimization would ``freeze'' upon random choices of local
