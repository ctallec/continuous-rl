...
towards $s'$. It is defined by the Bellman equation
\begin{equation}
...
\end{equation}
although it's not clear to me how to exploit the last expression. But I
think we have to work from both ends if we want to represent
compositionality of actions... (With the first equation, different values
...
solve a number of RL problems). Then we can learn an approximate
decomposition of the actual reward over the $\Psi_i$, and we know what to
do. (This assumes the reward is well approximated by the $\Psi_i$; or
that we learn the complement.) Advantage = even if there is no reward
signal for some time, we still learn to navigate the environment.
