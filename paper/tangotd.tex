...
This contains: Section 1: remarks on recovering the known convergence results
...
Section 2: the problem of the existence of near-solutions to the Bellman
equation that are very far from the true value function (I think this is
quite serious off-policy).

Section 3: Ideas on how, even if there's no reward signal, we should
...
\paragraph{Double sampling problem: using a model for one of the
...
\Delta}\rho R$. Moreover, \emph{if rewards during $s\to s'$ are
independent from $s'$}, we can sample from $\transp{\Delta}\rho R$. From
now on, I will assume that the reward collected during a transition $s\to
s'$ depends only on $s$. \footnote{This means the state $s$ is a
sufficient statistic. If the reward depends only on $s'$ instead, we can
just
shift the rewards by one step and pretend they are collected at the next
step; therefore we are back to a reward that depends only on $s$. The
only case not covered is if the reward really depends on the pair
$(s,s')$; in that case, the state $s'$ is not quite a sufficient
statistic of the process.

In the latter case, one would also need a model $\tilde R$ of rewards,
and then we could sample from $\transp{\tilde \Delta}\rho R$ and from
$\transp{\Delta}\rho\tilde R$.}


Let us consider the norm
\begin{equation}
\norm{\Delta V-R}^2_{P,\tilde P}\deq \transp{(\tfrac12 (\Delta+\tilde
\Delta)
V-R)}\rho(\tfrac12 (\Delta+\tilde \Delta)V-R)
\end{equation}
which is the $L^2(\rho)$-norm of the Bellman gap of an environment
$\frac12 (P+\tilde P)$ in which half of the transitions are real or
simulated.

The gradient of this norm is
\begin{equation}
\tfrac12 \transp{\Phi}\transp{(\Delta+\tilde
\Delta)
}\rho((\Delta+\tilde \Delta)V-R)
\end{equation}
