...
\label{sec:framework}
  ...
  Many reinforcement learning environments are not intrinsically time
	...
	Mujoco environments~\cite{ddpg} or OpenAI Gym classic\TODO{CITE!} control
	environments are discretization of continuous time control problems,
        with a discretization timestep $\deltat$ fixed by the simulator.
	For real world environments, the discretization timestep, $\deltat$ is
	most often imposed by the quality of sensors and actuators. As $\deltat$ is not an
...
%       }
	...
	actions. Consider the continuous \emph{Markov Decision Process} (MDP) defined by the \emph{Stochastic Differential Equation} (SDE)\TODO{Explain the intuition of Brownian in SDE}:
        \begin{equation}
		\label{eq:diffusion}
		ds = F(s, a)dt + \Sigma(s, a)\dbt.
              \end{equation}
	...
	M}_\deltat = \langle {\cal S}, {\cal A}, T_{\deltat}, r_\deltat,
      \gamma_\deltat\rangle$ is a discretization of the continuous MDP with \emph{time discretization} $\deltat$. The transition probability $T_\deltat(s, a, s')$ is the probability distribution of $s_\deltat$, when starting at $s_0 = s$ and maintaining $a_t=a$ constant for a time $\deltat$.
      \footnote{Informally, $T_\deltat(s, a, s')$ is approximately the probability distribution of $s + F(s, a)\deltat + \sqrt{\deltat}\Sigma(s, a){\cal N}(0, I)$, which is the discretization of Eq. \eqref{eq:diffusion} \TODO{CITE}}. This corresponds to an agent evolving in the continuous environment described by Eq. \eqref{eq:diffusion}, but who is only making observations and choosing actions every $\deltat$. We say the the MDP ${\cal M}_\deltat$ is near-continuous. 
	...
	\begin{theorem}
		...
		get observations every $\deltat$ \TODO{explain better}.
...
\label{sec:ret-gamma-v}
	...
	In what follows, $\deltat$-dependant definitions of the discount factor $\gamma_\deltat$
...
\begin{align}
  ...
  =& \E_{\tau\sim\pi}\left[\int\limits_{0}^\infty \gamma^{t}\deltat r_{t} \mid s_0 = s\right].
\end{align}
...
\begin{align}
  ...
  =& \E_{\tau\sim\pi}\left[\sum\limits_{k=0}^\infty \gamma^{k\deltat}\deltat r_{k\deltat} \mid s_0 = s\right]
\end{align}
...
If the MDP is following the Eq. \eqref{eq:diffusion}, the limit of the Bellman equation for $V^\pi_\deltat$ when $\deltat \rightarrow 0$ is the Hamilton-Jacobi-Bellman equation on $V^\pi$:\TODO{Cite ?}
\begin{equation}
...
for a continuous MDP. More precisely, the Q-function  collapses to the V-function when $\deltat \rightarrow 0$. In near continuous time,
the effect of individual actions on the Q-function is of order $\bigO(\deltat)$. This makes deriving a hierarchy of action from an approximate Q-function difficult.
Formally:
\begin{theorem}
  For all $s, a$, we have
  \begin{equation}
