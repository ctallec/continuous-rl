...
regular policy, and $\bar{A}$ and $\bar{V}$ be regular function approximators.}
	...
	\begin{itemize}
		...
		\item If $\beta < 1$, parameters grow arbitrarily large after an arbitrarily small physical time when $\deltat$ goes to $0$.		\item If $\beta > 1$, parameter trajectories become
		stationary as
			$\deltat$ goes to $0$.
	\end{itemize}
