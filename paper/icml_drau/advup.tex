...
to an action dependent quantity when $\deltat$ goes to $0$ \LB{In next equation, tiny detail but I would write $\frac{1}{\deltat}(r\deltat+...)$, easier to read.}
\begin{align}
...
\LB{It might be my math bias, but I would really prefer to read something like:
  Contrary to the action state value function, the rescaled advantage function converges
  to an action dependent quantity when $\deltat$ goes to $0$, which contains all the necessary information to improve the strategy. We have the following theorem:
  \begin{theorem}
    There is function $A^\pi(s, a)$ such that for all $a, s$, we have:
    \begin{equation}
      A^\pi_\deltat(s, a) \rightarrow_{\deltat \rightarrow 0} A^\pi(s, a)
    \end{equation}
    Moreover, $A^\pi$ contains all the necessary information about actions. We have $\pi$ is the optimal strategy if and only if $A^\pi(s, \pi(s)) = 0$ for all $s \in {\cal S}$.
    [[I don't know if this is true, but this is the kind of property we want for A, and this is the idea of not only it converges, but it converges to a meaningful limit]]
  \end{theorem}
  And the equation which is there before would only be in the appendix, in the proof.
}
...
\LB{All this subsection is not extremely clear to me, especially the next paragraph. And it is a detail, but I'm not sure I like $\nu$ as a notation for a random variable.} A sufficient condition for the exploratory policy to admit a time continuous
...
$A^\pi_\deltat(s, \pi_\phi(s))$ in the continuous case.  \LB{Maybe say here that it's the same than DDPG?}Eq.~\eqref{eq:max_A}
...
\end{align}
\LB{I don't like the notations $\alpha^1$ and $\alpha^2$. What about $\alpha^A$ and $\alpha^V$? Easier to read} 
