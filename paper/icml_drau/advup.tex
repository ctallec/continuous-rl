...
\TODO{
\begin{theorem}
Under suitable smoothness assumptions $A^\pi_\deltat(s,a)$ converges to a
limit $A^\pi(s,a)$ when $\deltat\to 0$. The limit is informative about
actions, namely, if policy $\pi'$ satisfies $V^{\pi'}>V^{\pi}$ for all states, then
$A^\pi(s,\pi'(s))>0$ for some $s$.
\end{theorem}
}
...
regular policy, and $\bar{A}$ and $\bar{V}$ be regular function approximators.}
	...
	\begin{itemize}
		...
		\item If $\beta < 1$, parameters grow arbitrarily large in after an arbitrarily small physical time when $\deltat$ goes to $0$. \TODO{not quite true, in a single
			step the update is bounded. But the parameters
			become arbitrary large in an arbitrarily small
			physical time.}
