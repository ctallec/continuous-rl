...
Then the above uses TD to solve the modified Bellman equation with
rewards $r(s,a)-b-\alpha \E_{\rho^\pi}V$. Thus we are trying to solve
$r(s,a)-b-\alpha \E_{\rho^\pi}V+\gamma PV-V=0$, and the only fixed point is
\begin{equation}
...
V(s)=V^{\pi_A}(s)-\frac{b}{1-\gamma}
\end{equation}
considered above; however, these two may still coincide for the value of $b$ at
equilibrium.
...
\end{equation}
contrary to $b=(1-\gamma)\E V^{\pi_A}$ we had before. Let us check that
the constraint $\E V=0$ is satisfied:
\begin{equation}
\E V= \E V^{\pi_A} -\frac{b+\E_{\rho^\pi} V^{\pi_A}}{1+\alpha-\gamma}
\end{equation}
