...
class PendulumUtilities:
    ...
    def state_transition(self, s: np.ndarray, u: np.ndarray) -> np.ndarray:
    ...
    def max_transition(self, s: np.ndarray, dv: np.ndarray):
        """ take max_u(r(s, u) + V(s')). """
        return np.maximum(*[(dv * self.state_transition(s, k)).sum(axis=0) for k in (-1, 1)])
